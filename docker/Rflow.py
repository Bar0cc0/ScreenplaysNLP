...
class ConnectPSQL:
	...
	def __init__(self, conn_id):
		...
		self.host = self.conn.host or "postgres"
		self.login = self.conn.login or "airflow"
		self.password = self.conn.password or "airflow"
		self.port = self.conn.port or 5432
		self.database = self.conn.schema or "airflow"
...
load_taskchain = [
    ...
    {"task_type": "bash", "name": "insert_values", "script": """
        PGPASSWORD=airflow psql -h postgres -U airflow -d airflow -c "\\COPY {{ params.table_name }} FROM '{{ ti.xcom_pull(task_ids='process_data') }}' WITH CSV HEADER"
    """, "context": cs, "config": CONFIG},
]
...
with DAG(
	...
	load_success = ShortCircuitOperator(
